...
std::shared_ptr<AST::functionStatement>
parser::Parser::_parseFunctionStatement() {
    if (!this->_expectPeek(token::tokenType::Identifier)) {
        return nullptr;
    }
    auto name =
        std::make_shared<AST::identifierLiteral>(this->current_token->literal);
    ...
    while (this->current_token->type != token::tokenType::RightParen) {
        std::cout << (this->current_token->type ==
                      token::tokenType::Identifier);
        if (this->current_token->type == token::tokenType::Identifier) {
            auto identifier = std::make_shared<AST::identifierLiteral>(
            ...
            parameters.push_back(
                std::make_shared<AST::functionStatement::parameter>(identifier,
                                                                    type));
            this->_nextToken();
    ...
    return std::make_shared<AST::functionStatement>(name, parameters,
                                                    returnType, body);
}
...
std::shared_ptr<AST::blockStatement> parser::Parser::_parseBlockStatement() {
    ...
    while (!this->_currentTokenIs(token::tokenType::RightBrace) &&
           !this->_currentTokenIs(token::tokenType::EndOfFile)) {
...
parser::Parser::_parseExpressionStatement() {
    auto expr = this->_parseExpression(PrecedenceType::LOWEST);
    if (this->_peekTokenIs(token::tokenType::Semicolon)) {
        this->_nextToken();
    }
...
std::shared_ptr<AST::statement> parser::Parser::_parseVariableDeclaration() {
    auto identifier =
        std::make_shared<AST::identifierLiteral>(current_token->literal);
    this->_nextToken();
    this->_nextToken();
    auto type = this->_parseType();
    if (this->peek_token->type == token::tokenType::Semicolon) {
        this->_nextToken();
        return std::make_shared<AST::variableDeclarationStatement>(identifier,
                                                                   type);
...
std::shared_ptr<AST::baseType> parser::Parser::_parseType() {
    ...
    if (this->_currentTokenIs(token::tokenType::Integer))
        name = std::make_shared<AST::integerLiteral>(
            std::stoll(this->current_token->literal));
    else if (this->_currentTokenIs(token::tokenType::Float))
        name = std::make_shared<AST::floatLiteral>(
            std::stod(this->current_token->literal));
    else if (this->_currentTokenIs(token::tokenType::String))
        name =
            std::make_shared<AST::stringLiteral>(this->current_token->literal);
...
std::shared_ptr<AST::statement> parser::Parser::_parseVariableAssignment() {
    ...
    this->_nextToken();
    auto stmt =
        std::make_shared<AST::variableAssignmentStatement>(identifier, expr);
    return stmt;
}

std::shared_ptr<AST::expression>
parser::Parser::_parseExpression(PrecedenceType precedence) {
    ...
    while (!_peekTokenIs(token::tokenType::Semicolon) &&
...
std::shared_ptr<AST::expression> parser::Parser::_parseInfixExpression(
    std::shared_ptr<AST::expression> leftNode) {
    auto infixExpr = std::make_shared<AST::infixExpression>(
        leftNode, this->current_token->type);
...
std::shared_ptr<AST::expression> parser::Parser::_parseIntegerLiteral() {
    auto expr = std::make_shared<AST::integerLiteral>(
        std::stoll(current_token->literal));
    return expr;
}

std::shared_ptr<AST::expression> parser::Parser::_parseFloatLiteral() {
    auto expr =
        std::make_shared<AST::floatLiteral>(std::stod(current_token->literal));
    return expr;
}
...
void parser::Parser::_peekError(token::tokenType type,
                                token::tokenType expected_type,
                                std::string suggestedFix) {
    std::shared_ptr<errors::SyntaxError> error =
        std::make_shared<errors::SyntaxError>(
            this->lexer->source, *peek_token,
            "Expected next token to be " +
                *token::tokenTypeString(expected_type) + ", got " +
...
void parser::Parser::_noPrefixParseFnError(token::tokenType type) {
    std::shared_ptr<errors::NoPrefixParseFnError> error =
        std::make_shared<errors::NoPrefixParseFnError>(
            this->lexer->source, *peek_token,
            "No prefix parse function for " + *token::tokenTypeString(type));
    this->errors.push_back(error);
}