...
void errors::Error::raise(bool terminate) {
    ...
    if (st_line != -1 && end_line != -1) {
        ...
        if (st_line - 1 > 0) {
            std::cerr << "\033[0;32m" << st_line - 1 << " | \033[0m"
                      << getStringOnLineNumber(source, st_line - 1) << "\n";
        }
        for (int i = st_line; i <= end_line; i++) {
            std::cerr << "\033[0;32m" << i << " | \033[0m"
                      << getStringOnLineNumber(source, i) << "\n";
        }
        if (end_line + 1 <= getNumberOfLines(source)) {
            std::cerr << "\033[0;32m" << end_line + 1 << " | \033[0m"
                      << getStringOnLineNumber(source, end_line + 1) << "\n";
        }
    }
...
void errors::NoPrefixParseFnError::raise(bool terminate) {
    ...
    if (token.line_no - 1 >= 0) {
        std::cerr << "\033[0;32m" << token.line_no - 1 << " | \033[0m"
                  << getStringOnLineNumber(source, token.line_no - 1) << "\n";
    }
    int litLineNo = token.line_no;
    std::stringstream concatenatedLines;
    std::string line;
    while (std::getline(std::istringstream(token.literal), line)) {
        concatenatedLines << (litLineNo == token.line_no
                                  ? "\033[1;31m\033[4m"
                                  : "\033[0;32m" + std::to_string(litLineNo) +
                                        " | \033[1;31m\033[4m")
                          << line << "\033[0m\n";
        litLineNo++;
    }
    std::string literal = concatenatedLines.str();
    line = (getStringOnLineNumber(source, token.line_no)
                .substr(0, token.col_no - 1) +
            literal + "\033[0m");
    std::cerr << "\033[0;32m" << token.line_no << " | \033[0m" << line << "\n";

    if (token.line_no + 1 <= getNumberOfLines(source) &&
        litLineNo == token.line_no) {
        std::cerr << "\033[0;32m" << token.line_no + 1 << " | \033[0m"
                  << getStringOnLineNumber(source, token.line_no + 1) << "\n";
    }
...
void errors::raiseSyntaxError(const std::string &source,
