...
std::shared_ptr<std::string> Lexer::_peekChar(int offset) {
    if ((this->pos + offset) >= static_cast<int>(this->source.length())) {
        ...
        return std::make_shared<std::string>(1,
                                             this->source[this->pos + offset]);
    }
}

std::shared_ptr<token::Token> Lexer::_newToken(token::tokenType type,
                                               std::string currentChar) {
    auto x = std::make_shared<token::Token>(type, currentChar, this->lineNo,
                                            this->colNo);
    return x;
}
        ...
        if (this->currentChar == ".") {
            ...
            if (dot_count > 1) {
                printf("Invalid number at line %u, column %i\n", this->lineNo,
                       this->colNo);
                return this->_newToken(token::tokenType::Illegal,
                                       this->currentChar);
            }
        }
...
std::shared_ptr<std::string> Lexer::_readIdentifier() {
    ...
    while (this->_isLetter(this->currentChar) ||
           this->_isDigit(this->currentChar)) {
...
std::string getStringOnLineNumber(const std::string &str, int lineNumber) {
    ...
    return str.substr(startPos, (endPos != std::string::npos)
                                    ? endPos - startPos
                                    : std::string::npos);
}
