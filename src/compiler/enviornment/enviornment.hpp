...
enum class RecordType { RecordClassType, RecordVariable, RecordFunction };
...
class RecordFunction : public Record {
  public:
    ...
    std::shared_ptr<RecordClassType> return_type;
    ...
    RecordFunction(std::string name, llvm::Function* function, llvm::FunctionType* function_type,
                   std::vector<std::tuple<std::string, std::shared_ptr<RecordVariable>>> arguments, std::shared_ptr<RecordClassType> return_type)
...
class RecordClassType : public Record {
  public:
    ...
    RecordClassType(std::string name, llvm::StructType* struct_type, std::vector<std::string> variable_names,
                    std::unordered_map<std::string, std::shared_ptr<RecordFunction>> functions = {})
        : Record(RecordType::RecordClassType, name), struct_type(struct_type), variable_names(variable_names) {};
    RecordClassType(std::string name, llvm::Type* stand_alone_type)
        : Record(RecordType::RecordClassType, name), stand_alone_type(stand_alone_type) {};
};
